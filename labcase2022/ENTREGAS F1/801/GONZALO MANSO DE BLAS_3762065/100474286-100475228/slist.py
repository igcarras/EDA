...
This notebook implements a singly linked list, which only saves the reference to the first node and the _size of the list.
"""
...
if start == 0:
    ...
    while posicion != start:
        ...
        posicion += 1
    primernodo = cabeza
    primernodo.next = inputList._head

while posicion <= end:
    cabeza = cabeza.next
    posicion += 1
nodofinal = cabeza.next

cola = inputList._head
while cola.next != None:
    cola = cola.next
cola.next = nodofinal
'''
