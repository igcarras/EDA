...
class Archipelago:
    ...
    def accessible_from(self, i: int, h: int) -> list:
        accesibles = []
        if i not in self._adjacent_islands:
            return []
        while len(accesibles) < len(self._adjacent_islands):      
            for v in self._neigbours(i):
                if v._height <= h:
                    accesibles.append(v)
                    i = v
        return accesibles
    ...
    def minimum_path(self, start: int, end: int, tide : int = 0) -> (list, int):
        ...
        if end not in self.accessible_from(start, tide):
            return [], math.inf
        else:
            encontrado = False
            i = 0
            lista = []
            camino = 0
            while not encontrado and i < len(self.accessible_from(start, tide)):
                if self.accessible_from(start, tide)[i] == end:
                    encontrado = True
                else:
                    lista.append(self.accessible_from(start, tide)[i])
                    camino += self.accessible_from(start, tide)[i]._distance
                    i += 1
            lista.append(end)
