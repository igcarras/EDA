...
class Archipelago:
    ...
    def accessible_from(self, i: int, h: int) -> list:
        ...
        if i not in self.lst_neigbours.keys():
            print(i, ' does not exist!!!')
            return None, None
        if h not in self.lst_neigbours.keys():
            print(h, ' does not exist!!!')
            return None, None

        i, h = self.lst_neigbours(i)
        if i[h] == math.inf:
            return [], math.inf
    ...
    def minimum_path(self, start: int, end: int, tide : int = 0) -> (list, int):
        ...
        for i in self.lst_neigbours.keys():
            if start[i] == sys.maxsize:
                print("There is not path from ", tide, ' to ', i)
            else: 
                minimum_path = []
                prev = end[i]
                while prev is not None:
                ...
                print(tide, '->', i, ":", minimum_path, start[i])
