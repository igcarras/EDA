...
class Archipelago:
    ...
    def accessible_from(self, i: int, h: int) -> list:
        neighbors=self._neigbours(i)
        print(neighbors)

        
        for a in range(len(neighbors)):
            #print(i)
            print(self._adjacent_islands[i][a]) 

            #con esto tengo las tuplas (triplas?) de las islas adjacentes a la isla intruducida. Habría que comparar el segundo
            #elemento de la tupla con la altura h introducida. No recuerdo como acceder al elemento con indice 1 en este tipo de
            #estructura
        


    def minimum_path(self, start: int, end: int, tide : int = 0): #no me ha dado tiempo, pero aparenta ser un dijkstra modificado
        ...
