...
class Archipelago:
    ...
    def accessible_from(self, i: int, h: int) -> list:
        ...
        while len(queue) > 1:
            s = queue.pop(0)
            for x in self._neigbours(s):
                x_height = x[1]
                if x_height > h and x not in aux:
                    aux.append(x)
                    queue.append(x)
    ...
    def minimum_path(self, start: int, end: int, tide : int = 0) -> (list, int):
        self._djikstra(start)
        
            
        
    def _minDistances(self, distances, visited):
        min = sys.maxsize
    ...
    def _djikstra(self, origin):
        ...
        for n in range(len(self._adjacent_islands)):
            u = self.minDistances(distances, visited)
            visited[u] = True
            for adj in self._neigbours(u):
                w = adj[1]
                if visited[adj] == False and distances[adj] > distances[u] + w:
                    distances[adj] = distances[u] + w
                    previous[adj] = u
