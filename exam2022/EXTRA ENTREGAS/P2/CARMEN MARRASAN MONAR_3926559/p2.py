...
class BinaryTree:
    ...
    def right_sum(self) -> int:
        ...
        if self._root is None:
            return 0
        result = 0
        return self._right_sum(self._root,result)
    
    def _right_sum(self,node,result):
        if node:  
            if node.right:
                result += node.elem
                result = self._right_sum(node.right,result)
            if not node.right and node.left:
                result = self._right_sum(node.left,result)
                if not node.right:
                    result += node.elem
