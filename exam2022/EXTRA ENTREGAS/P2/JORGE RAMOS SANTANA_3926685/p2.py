...
class BinaryTree:
    ...
    # Define a variable to track current level
        ...
        result=[]
        total=0
        self._right_sum(self._root,result,nivel)
        for i in range(len(result)-1):
            total+=result[i]
        print(result)
        return total

    def _right_sum(self,node,result,nivel):
        if node!=None:
            result.append(node.elem)
            self._right_sum(node.right,result,nivel+1)
            
            self._right_sum(node.left,result,nivel)
