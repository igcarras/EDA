...
class BinaryTree:
    ...
    def _right_sum(self, node):
        if node.left is None and node.right is None:
            return node.elem
        suma = 0 
        lista_nodos =self.level_order_list()
        lista_nivel = []
        for i in range(self.height):
            lista_nivel.append([])
            
        for i in lista_nodos:
            altura = self._height(i)
            lista_nivel.append[altura]
        for i in lista_nivel:
            suma += max(i)
        return suma
