...
class BinaryTree:
    ...
    def right_sum(self) -> int:
        ...
        if self._root == None:
            return 0
        nodo = self._root
        return self._right_sum(nodo, suma)
    
    def _right_sum(self, nodo, suma):
        suma += nodo.elem
        while nodo.left and nodo.right:
            suma += nodo.right.elem
            nodo = nodo.right
        
        while nodo.right and not nodo.left:
            suma += nodo.right.elem
            nodo = nodo.right
        
        while nodo.left and not nodo.right:
            suma += nodo.left.elem
            nodo = nodo.left
        return suma
