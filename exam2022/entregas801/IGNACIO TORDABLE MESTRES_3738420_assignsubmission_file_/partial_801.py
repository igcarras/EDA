...
class MySList():
    ...
    def subtraction(self, other):
        ...
        if self.isSorted() == False or other.isSorted() == False:
            ...
            longitud2 = 0
            # Medir longiudes de ambas listas
            while current1:
                longitud1 += 1
            ...
            if longitud2 == longitud1:
                # Restar todos uno a uno porque tienen la misma longitud
                for a in range(longitud1):
                    listaResta.append(abs(current1.elem - current2.elem))
                    current1 = current1.next
                    current2 = current2.next
            elif longitud1 > longitud2:
                resta = longitud1 - longitud2
                # Restar el número de elementos que tienen en común
                for a in range(longitud2):
                    listaResta.append(abs(current1.elem - current2.elem))
                    current1 = current1.next
                    current2 = current2.next
                # Añadir a la lista los que quedan
                for a in range(resta):
                    listaResta.append(current1.elem)
                    current1 = current1.next
            else:  # longitud2 > longitud1
                resta = longitud2 - longitud1
                for a in range(longitud1):
                    listaResta.append(abs(current1.elem - current2.elem))
                    current1 = current1.next
                    current2 = current2.next
                for a in range(resta):
                    listaResta.append(current2.elem)
                    current2 = current2.next
            return listaResta
