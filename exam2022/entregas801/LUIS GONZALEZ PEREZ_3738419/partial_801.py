...
class MySList():
    ...
    def subtraction(self, other):
        ...
        if self.isSorted() and other.isSorted():
            ...
            #Mientras haya elementos en ambas listas realizamos la resta
            while node2 != None and node1 != None:
                newlist.append(abs(node1.elem - node2.elem))
                node1 = node1.next
                node2 = node2.next
            
            #Una vez no se pueden realizar más restas, se añade cualquier elemento restante de la lista mayor
            while node1 != None:
                newlist.append(node1.elem)
                node1 = node1.next
            while node2 != None:
                newlist.append(node2.elem)
                node2 = node2.next
                
            return newlist
        else:
            #Si las listas no está ordenadas no se ejecuta el método
            return None
