...
class MySList():
    ...
    def subtraction(self, other):
        ...
        # Devolver None si no estan ordenadas ambas listas
        if not (self.isSorted() and other.isSorted()):
            return None
        else:
            # Creamos punteros para cada una de las listas
            aux1 = self._head
            aux2 = other._head

            while aux1 or aux2:
                if not aux1:
                    self.append(aux2.elem)
                else:
                    if not aux2:
                        return self
                    else:
                        aux1.elem = abs(aux1.elem - aux2.elem)
                
                # Avanzamos en cada lista

                if not aux1:
                    aux1 = self._head.next
                else:
                    aux1 = aux1.next
