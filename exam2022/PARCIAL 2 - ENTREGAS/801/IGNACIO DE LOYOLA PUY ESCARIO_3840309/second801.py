...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    ...
    def _removeInsideRange(self, node: BinaryNode, min, max, lista_aux):
        if node:
            self._removeInsideRange(node.right, min, max, lista_aux)
            if node.left is None and node.right is None:
                if node.elem >= min and node.elem <= max:
                    lista_aux.append(node.elem)
                    return None
            self._removeInsideRange(node.left, min, max, lista_aux)
