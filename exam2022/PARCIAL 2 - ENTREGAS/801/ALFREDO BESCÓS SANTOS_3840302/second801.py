# -*- coding: utf-8 -*-
"""Alfredo Bescós Santos grupo 801"""
class BinaryNode:
...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, min: int, max: int) -> []:
        result = []
        if min > max:
            print('El intervalo no está bien definido, introduzca un min menor que el max')
        if self._root is None:
            print('Arbol vacio')
        result = self._removeInsideRange(self._root, min, max, result)
        return result
    
    def _removeInsideRange(self, node: BinaryNode, min: int, max:int, nodeList: list) -> []:
        
        if node:
            self._removeInsideRange(node.right, min, max, nodeList)
            self._removeInsideRange(node.left, min, max, nodeList)
