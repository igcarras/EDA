...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, min: int, max: int) -> []:
        return self._removeInsideRange(self._root,[])
    
    def _removeInsideRange(self, node,lista):
        if self._root==None:
            return None
        
                
        if self._root!=None and node.right==None and node.left==None:
             lista.append(node.elem)
                
        if self._root:
            
            if node.right: 
                    lista.append(node.right, node.elem)
                    lista.removeInsideRange(node.right, node.elem)
                         
                 
            if node.left:
                lista.append(node.left, node.elem)
                lista.removeInsideRange(node.left, node.elem)
