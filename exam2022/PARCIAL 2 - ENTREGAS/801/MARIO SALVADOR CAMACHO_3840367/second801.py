...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    ...
    def removeInsideRange(self, min:int, max:int) -> []:
        resultado = []
        self._removeInsideRange(self._root, min, max, resultado)
        return resultado
    
    def _removeInsideRange(self, node:BinaryNode, min:int, max:int, lista:list) -> None:
        if node:
            if node.right is None and node.left is None: 
                if node.elem >= min and node.elem <= max:
                    lista.append(node.elem)
                    return None                
            self._removeInsideRange(node.left, min, max, lista)    
            self._removeInsideRange(node.right, min, max, lista)
