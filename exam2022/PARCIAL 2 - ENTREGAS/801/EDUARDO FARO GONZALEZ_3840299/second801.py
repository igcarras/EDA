...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, min: int, max: int) -> []:
        ...
        return lista
    def _removeInsideRange(self,node, nmin, nmax, listahojas):
        if node:
            self._removeInsideRange(node.right, nmin, nmax, listahojas)
            if node.left==None and node.right==None and nmin<=node.elem<=nmax:
                listahojas.append(node.elem)
                return None
            self._removeInsideRange(node.left, nmin, nmax, listahojas)
