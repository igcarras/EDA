...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    ...
    """Definimos _removeInsideRange para poder meter un nodo en el (empezamos por la raiz del arbol).
   Dentro recorreremos todos los nodos empezando por la derecha y si encuentra una hoja comprueba que esta esté dentro de min,max
   si lo está la añade a la lista result y la borra del arbol, por recursion esto pasa al anterior que buscara Hojas cada vez más a la izquierda
   haciendo que al hacer append la lista quede ordenada de mayor a menor"""

    def _removeInsideRange(self,node, min:int, max:int, result:list) -> []:

        if node.right:
            self._removeInsideRange(node.right,min,max,result)
        if node.left:
            self._removeInsideRange(node.left,min,max,result)
