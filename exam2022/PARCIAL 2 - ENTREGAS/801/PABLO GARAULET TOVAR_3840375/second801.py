...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    ...
    def _removeInsideRange(self, puntero, min: int, max: int, ascen, lista):
        if puntero == None: #vacia
            return lista

        if puntero.left == None and puntero.elem <= max and puntero.elem >= min and puntero.right == None:
            if ascen == puntero and ascen.left == puntero:
                lista.append(puntero.elem)
                ascen.left.elem = None

                return lista

            if ascen and ascen.right == puntero:
                lista.append(puntero.elem)
                ascen.right.elem = None
        ...
        self._removeInsideRange(puntero.right, min, max, puntero, lista)
        self._removeInsideRange(puntero.left, min, max, puntero, lista)
        return lista
    ...
    def removeInsideRange(self, min: int, max: int) -> []:
            ...
            self._removeInsideRange(self._root, min, max, None, lista)
            return lista
