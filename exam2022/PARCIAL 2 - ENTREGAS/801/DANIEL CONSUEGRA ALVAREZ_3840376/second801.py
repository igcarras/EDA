...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def findnodoshoja(self):
        auxList = []
        auxList.append(self._findnodoshoja(self._root))
        return auxList
    def _findnodoshoja(self,node):
        if node.right is None and node.left is None:
            return node
        elif node.left is not None:
            return self._findnodoshoja(node.left)
        elif node.right is not None:
            return self._findnodoshoja(node.right)
    def minimo(self):
        return self._minimo(self._root)
    def _minimo(self,node):
        if node is None:
            return node
        else:
            return self._minimo(node.left)
    ...
    def _remove(self, node, elem):
        ...
        if elem < node.elem:
            ...
            if node.left is None and node.right is None: #nodo hoja
                return None

            if node.left is None:  # solo tiene hijo derecho
                return node.right

            elif node.right is None:  # solo tiene hijo izquierdo
                return node.left
            else:
                successor = self._minimo(node.right)
    ...
    def removeInsideRange(self, min: int, max: int) -> []:
        removedlist=[]
        for node in self.findnodoshoja():
            if node.elem>=min and node.elem<=max:
                removedlist.append(node.elem)
                self._remove(node,node.elem)
        else:
            return removedlist
