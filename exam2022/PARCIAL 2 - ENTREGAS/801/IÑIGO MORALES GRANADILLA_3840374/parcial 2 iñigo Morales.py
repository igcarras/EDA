...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, min: int, max: int) -> []:
        eliminados = []
        self._removeInsideRange(self._root, min, max, eliminados)
        return eliminados
    
    def _removeInsideRange(self, node: 'BSTNode', min, max, eliminados):
        if node == None:
            return eliminados
        if node.right or node.left:
            return self._removeInsideRange(node.right, min, max, eliminados) and self._removeInsideRange(node.left, min, max, eliminados)
        if node.right == None and node.left == None:
            eliminados.append(node.elem)
            
            for i in range(len(eliminados)):
                if eliminados[i] > max or eliminados[i] < min:
                    eliminados.remove(eliminados[i])
            sorted(eliminados)
            return eliminados
