  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, min: int, max: int) -> []:
        ...
        if min == max:
            ...
            lista = self._removeInsideRange(min, max, self._root, [])
            print(lista)
            return lista

    def _removeInsideRange(self, min: int, max: int, node: BinaryNode, lista: list)->list:
        if not node:
            return lista
        if not node.left and not node.right: #node hoja
            ...
            self._removeInsideRange(min, max, node.right, lista)
            self._removeInsideRange(min, max, node.left, lista)
        return lista
