...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    ...
    def removeInsideRange(self, min: int, max: int) -> list:
        lista = []     #creamos la lista que posteriormente llenarmos
        self._removeInsideRange(self._root, min, max, lista)
        return lista  #devolvemos la lista ya creada

    def _removeInsideRange(self, node: BinaryNode, min, max, lista): #creamos el nodo auxiliar y el segundo metodo con la recurrencia
        if node == None:   #si el arbol esta vacio devolvemos la lista sin ningun elemento
            return lista
        if node.left or node.right:    #comprobamos si tiene hijos
            return self._removeInsideRange(node.left, min, max, lista) and self._removeInsideRange(node.right, min, max, lista)
