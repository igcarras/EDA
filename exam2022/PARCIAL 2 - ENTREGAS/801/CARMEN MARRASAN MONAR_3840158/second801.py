...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, min: int, max: int) -> []:
        ...
        return result
    def _removeInsideRange(self, node:BinaryNode, min:int, max:int, lst:list) -> None:
        if node:
           nodo_hoja = False
           if node.left is None and node.right is None:
               nodo_hoja = True
           node.right = self._removeInsideRange(node.right, min, max, lst)
           if nodo_hoja:
               if min <= node.elem and node.elem<= max:  # Mira si es nodo hoja y si su elem está en el rango
                   lst.append(node.elem)
                   return  None 
           node.left = self._removeInsideRange(node.left, min, max, lst)
           return node
        return node
