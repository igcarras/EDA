#Guillermo Hijas Serrano Grupo 801
# -*- coding: utf-8 -*-
...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, minimo: int, maximo: int) -> []:
        lista = []
        self._removeInsideRange(self._root,minimo,maximo,lista)
        return lista
    def _removeInsideRange(self, node:BinaryNode, minimo:int, maximo:int, lista:list) -> None:
        if node:
            self._removeInsideRange(node.right, minimo, maximo, lista)
            if node.right is None and node.left is None and minimo<=node.elem<=maximo:
                lista.append(node.elem)
                return None
            self._removeInsideRange(node.left, minimo, maximo, lista)
