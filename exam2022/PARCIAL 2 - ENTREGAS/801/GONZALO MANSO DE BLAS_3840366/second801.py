...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, min: int, max: int) -> []:
        ...
        return result
    def _removeInsideRange(self, node:BinaryNode, min, max, result):
        if node == None:
            return result
        if node.right or node.left:
            return self._removeInsideRange(node.right, min, max, result) and self._removeInsideRange(node.left, min, max, result)
