...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    ...
    def removeInsideRange(self, min: int, max: int) -> []:
        resultado = []
        self._root = self._removeInsideRange(self._root, min, max, resultado)
        return resultado

    def _removeInsideRange(self, node, min, max, resultado):
        if node != None:
            leaf = False
            if not node.right and not node.left:
                leaf = True
            node.right = self._removeInsideRange(node.right, min, max, resultado)
            if leaf == True:
                if min <= node.elem <= max:
                    resultado.append(node.elem)
                    return None
            node.left = self._removeInsideRange(node.left, min, max, resultado)
            return node
        return node
