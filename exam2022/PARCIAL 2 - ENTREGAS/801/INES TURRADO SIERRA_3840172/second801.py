...
class MyBinarySearchTree:
  ...
  # returns a sorted list in descending order
    def removeInsideRange(self, min: int, max: int) -> []:
        lista_resultado = []
        self._removeInsideRange(self._root, min, max, lista_resultado)
        return lista_resultado

    def _removeInsideRange(self, node: BinaryNode, min: int, max: int, lista_nodos: list) -> []:
        if node:
            self._removeInsideRange(node.right, min, max, lista_nodos) #Función (recurrencia) rama derecha

            #Si es no tiene hijos (es un nodo hoja) comprueba si está dentro del rango
            if node.left is None and node.right is None:
                if node.elem <= max and node.elem >= min:
                    lista_nodos.append(node.elem)
                    return None

            self._removeInsideRange(node.left, min, max, lista_nodos) #Función rama izquierda
