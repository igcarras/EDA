...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        lista_nodos = []
        nodo_aux1 = self._root()
        medio = max//2
        if min == 0 and max == 0:
            return lista_nodos
        
        if nodo_aux1.elem >= min and nodo_aux1.elem <= medio:
            nodo_aux1.left = self.removeOutsideRange(min, max)

        elif nodo_aux1.elem > medio and nodo_aux1.elem <= max:
            nodo_aux1.right = self.removeOutsideRange(min, max)
            
        else:
            if nodo_aux1.left == None and nodo_aux1.elem < min:
                lista_nodos.append(nodo_aux1.elem)
                
            elif nodo_aux1.left == None and nodo_aux1.elem > max:
                lista_nodos.append(nodo_aux1.elem)
