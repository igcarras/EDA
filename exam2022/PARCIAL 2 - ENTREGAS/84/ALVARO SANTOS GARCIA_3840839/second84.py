...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        lista = []
        padre = self._root
        self._removeOutsideRange(min, max, self._root, lista, padre)
        return lista

    def _removeOutsideRange(self, min, max, node, lista, padre):
        if node == None:
            return None
        elif node.right == None and node.left == None:
            if node.elem > max or node.elem < min:
                lista.append(node.elem)
                if padre == node == self._root:
                    self._root = None
            ...
            if node != self._root:
                padre = node
            self._removeOutsideRange(min, max, node.left, lista, padre), self._removeOutsideRange(min, max, node.right, lista, padre)
