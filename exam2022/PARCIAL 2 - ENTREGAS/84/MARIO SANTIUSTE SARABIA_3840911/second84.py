...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        lista = self._removeOutsideRange(self._root, min, max, [])
        self.delete_leaves(lista, self._root)
        return lista

    def _removeOutsideRange(self, node: BinaryNode, min: int, max: int, lista: list):
        if node:
            self._removeOutsideRange(node.left, min, max, lista)
            if not min <= node.elem <= max and not node.left and not node.right:
                lista.append(node.elem)
            self._removeOutsideRange(node.right, min, max, lista)
        return lista

    def delete_leaves(self, lista: list, node: BinaryNode):
        if node:
            self.delete_leaves(lista, node.left)
            if self._root.elem in lista:
                self._root = None
            if node.left and node.left.elem in lista:
                node.left = None
            if node.right and node.right.elem in lista:
                node.right = None
            self.delete_leaves(lista, node.right)
...
if __name__ == "__main__":
    ...
    for x in [50]:
        tree.insert(x)
    print("Remove leaf nodes out range: 5, 80")
    print("Nodes removed", tree.removeOutsideRange(51, 80))
    tree.draw()
