...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        result = []   # Una lista para meter lo resultados
        if self._root is None:
            return result
        else:
            self._removeOutsideRange(self._root,min,max,result)
            return result

    def _removeOutsideRange(self,node,min,max,list):
        if node.left: # Como queremos los menores primero hacemos los de la izquierda
            self._removeOutsideRange(node.left,min,max,list)
            if node.left.left is None:
                node.left = None

            if node.right:
                self._removeOutsideRange(node.right, min,max,list)
