...
class MyBinarySearchTree:
    ...
    def _removeOutsideRange(self, node, min: int, max: int, result) -> []:
        if node != None:
            if node.left == None and node.right == None:
                if min <= node.elem <= max:
                    ...
                    return None
            if node.left:
                node.left = self._removeOutsideRange(node.left, min, max, result)
            if node.right:
                node.right = self._removeOutsideRange(node.right, min, max, result)
        return node
