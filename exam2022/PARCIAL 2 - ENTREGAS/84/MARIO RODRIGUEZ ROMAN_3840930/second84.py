...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        aux=[]
        self._removeOutsideRange(self._root,min,max,None,aux)

        return aux

    def _removeOutsideRange(self,node,min:int,max:int,father,auxlist:list):
        if not node:
            return
        self._removeOutsideRange(node.left, min, max,node, auxlist)
        if not node.left and not node.right:
            if node.elem<min or node.elem>max:
                auxlist.append(node.elem)
                if father:
                    if node.elem<father.elem:
                        father.left=None
                    father.right=None
                node=None
                return

        self._removeOutsideRange(node.right,min,max,node,auxlist)
