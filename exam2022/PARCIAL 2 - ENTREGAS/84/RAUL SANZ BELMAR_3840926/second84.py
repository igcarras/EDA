...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        if min > max:
            min, max = max, min
        nodosRemovidos = []
        #El árbol sólo tiene root, y además el root está fuera del rango, por lo que lo eliminamos del árbol
        if self._root and not(self._root.left or self._root.right) and (self._root.elem < min or self._root.elem > max):
            nodosRemovidos.append(self._root.elem)
        #Árbol en el que no hay que eliminar el root, ya sea porque es un árbol vacío, y no se va a modificar la lista (ya que no hay nodos);
        #o porque el árbol continúa, entonces se recorrerá el árbol en busca de los nodos hoja fuera del rango.
        else:
            self._removeOutsideRange(self._root, min, max, nodosRemovidos)
        return nodosRemovidos

    def _removeOutsideRange(self, node:'BinaryNode', min:int, max:int, nodosRemovidos:list):
        if node is not None:
            if not(node.left or node.right) and (node.elem < min or node.elem > max):
                nodosRemovidos.append(node.elem)
                return None
            node.left = self._removeOutsideRange(node.left, min, max, nodosRemovidos)
            node.right = self._removeOutsideRange(node.right, min, max, nodosRemovidos)
        else:
            return None
