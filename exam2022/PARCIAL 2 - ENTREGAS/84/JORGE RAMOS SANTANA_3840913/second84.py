...
class MyBinarySearchTree:
    ...
    def _removeOutsideRange(self,node,min:int,max:int,lista):
        ...
        if node.left is None and node.right is None:
            if node.elem<min or node.elem>max:
                print("BORRAR",node.elem)
                lista.append(node.elem)
                return None

        if node:
            node.left=self._removeOutsideRange(node.left,min,max,lista)
            node.right=self._removeOutsideRange(node.right,min,max,lista)
