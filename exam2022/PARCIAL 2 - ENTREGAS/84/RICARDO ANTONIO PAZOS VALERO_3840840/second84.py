...
class MyBinarySearchTree:
    ...
    def _removeOutsideRange(self, min, max, node: BinaryNode, removed_leaves):
        if node is None:
            return []

        if node.left is None and node.right is None:
            if node.elem not in range(min, max):
                removed_leaves.append(node.elem)
                return removed_leaves

        self._removeOutsideRange(min, max, node.left, removed_leaves)
        self._removeOutsideRange(min, max, node.right, removed_leaves)
