...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        lista = []
        self._removeOutsideRange(self._root,min,max,lista)
        for n in lista:
            self.remove(n)
        #self.draw()
        return lista

    def _removeOutsideRange(self,node,min,max, lista):
        # Si existe los nodos se recorre el arbol si es un nodo hoja y esta fuera del rango se añade a la lista
        if node:
            self._removeOutsideRange(node.left,min,max,lista)
            if (node.elem < min or node.elem > max) and node.left is None and node.right is None:
                lista.append(node.elem)
                #print(node.elem)
            self._removeOutsideRange(node.right,min,max,lista)
        return lista

    def remove(self, elem):
        # metodo que borra los nodos hojas que estan fuera del rango
