...
class MyBinarySearchTree:
    ...
    def removeOutsideRange(self, min: int, max: int) -> []:
        lista_hojas = self.hojasFueraRango(self._root, min, max)
        for i in lista_hojas:
            self._root = self._remove(self._root, i)
        return lista_hojas

    def hojasFueraRango(self, nodo, min, max):
        return self._hojasMenores(nodo, min, []) + self._hojasMayores(nodo, max, [])

    def _hojasMenores(self, nodo, min, lista):
        if nodo is None:
            return
        elif nodo.elem < min:
            return nodo.elem
        else:
            lista.append(self._hojasMenores(nodo.left, min, []))
            return lista

    def _hojasMayores(self, nodo, max, lista):
        if nodo is None:
            return
        elif nodo.elem > max:
            return nodo.elem
        else:
            lista.append(self._hojasMayores(nodo.right, max, []))
            return lista

    def _remove(self, nodo, elem):
        if nodo is None:
            return nodo

        if elem < nodo.elem:
            nodo.left = self._remove(nodo.left, elem)
        elif elem > nodo.elem:
            nodo.right = self._remove(nodo.right, elem)
        else:
            return None
