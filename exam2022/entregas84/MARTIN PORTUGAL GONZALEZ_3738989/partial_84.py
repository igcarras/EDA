...
class MySList():
    ...
    def merge(self, other):
        ...
        if self.isSorted() == False or other.isSorted()== False : # 2º unitest. La lista
            return None

        elif self.isSorted() == True:
            if other.isSorted()== True:
                currentOther = other._head
                for i in range(len(other)):
                    siguiente = currentOther.next
                    if currentOther.elem == siguiente.elem:
                        currentOther.next = siguiente.next
                        other._size = other._size - 1
                        currentOther = currentOther.next
                    else:
                        currentOther = currentOther.next
                    return other
                    # metodo para comprobar si es duplicado
