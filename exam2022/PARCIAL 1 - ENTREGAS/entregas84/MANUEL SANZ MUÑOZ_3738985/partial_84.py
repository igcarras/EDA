...
class MySList():
    ...
    def merge(self, other):
        ...
        if self.isSorted() and other.isSorted():
            lista_nueva = MySList()
            last = None
            while self._head is not None or other._head is not None:
                if self._head is not None and (other._head is None or self._head.elem < other._head.elem):
                    if last is None or last < self._head.elem:
                        lista_nueva.append(self._head.elem)
                        last = self._head.elem
                    self._head = self._head.next
                elif self._head is None or self._head.elem > other._head.elem:
                    if last is None or last < other._head.elem:
                        lista_nueva.append(other._head.elem)
                        last = other._head.elem
