# coding=utf-8
# """Alfredo Bescós Santos, NIA: 100475112, grupo 801"""
...
class MySList():
    ...
    def subtraction(self, other):
        ...
        if not self.isSorted() or not other.isSorted():
            ...
            resultado = MySList()
            auxL = self._head
            auxOther = other._head
            while auxOther and auxL:
                resta = auxL.elem-auxOther.elem
                if resta < 0:
                    # si la resta es negativa, la convertimos en positiva
                    resta = resta*(-1)
                resultado.append(resta)
                auxL = auxL.next
                auxOther = auxOther.next
            # si sale del grupo o bien hemos recorrido toda la lista invocante
            # o toda la lista other
            if auxOther:
                # querría decir que no hemos recorrido toda la lista other
                while auxOther:
                    resultado.append(auxOther.elem)
                    # como la resta a None sería el valor en negativo de el elemento de other
                    # y no se puede tener valores negativos, añadimos directamente el elemento
                    auxOther = auxOther.next
            elif auxL:
                while auxL:
                    resultado.append(auxL.elem)
                    auxL = auxL.next
            return resultado
