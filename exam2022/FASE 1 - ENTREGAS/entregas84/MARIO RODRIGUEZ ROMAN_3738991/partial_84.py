...
class MySList():
    ...
    def merge(self, other):
        ...
        if not other.isSorted() or not self.isSorted():
            print("la lista tiene que estar ordenada")
        else:
            aux_list=MySList()
            aux=self._head
            aux_2=other._head
            while aux:
                while aux_2:
                    if aux.elem<aux_2.elem:
                        if aux_list._tail:
                            if aux_list._tail.elem<aux.elem:
                                aux_list.append(aux.elem)
                        else:
                            aux_list.append(aux.elem)
                    if aux.elem==aux_2.elem:
                        if aux_list._tail:
                    ...
                    if aux.elem>aux_2.elem:
                        if aux_list._tail:
                            if aux_list._tail.elem<aux_2.elem:
                                aux_list.append(aux.elem)
                        else:
                            aux_list.append(aux_2.elem)
                    aux_2=aux_2.next
                aux=aux.next
            aux_2 = other._head
            while aux_2:
                if aux_list._tail:
                    if aux_2.elem > aux_list._tail.elem:
                        aux_list.append(aux_2.elem)
                else:
                    aux_list.append(aux_2.elem)
                aux_2 = aux_2.next
