...
class MyBinarySearchTree:
    ...
    def _sumInsideRange4(self, node: BinaryNode, min: int, max: int, suma: int) -> int:
        ...
        if (node.left is not None) or (node.right is not None):
            if min <= node.elem <= max:
                suma += node. elem
        self._sumInsideRange4(node.left, min, max, suma)
        self._sumInsideRange4(node.right, min, max, suma)
    ...
    def sumInsideRangeSalvador(self, min: int, max: int) -> int:
        return self._sumInsideRangeSalvador(self._root, min, max)
    ...
    def _sumInsideRangeSalvador(self, node: BinaryNode, min: int, max: int) -> int:
        suma = 0
        if node is None:
            return 0

        if min<node.elem:
            self._sumInsideRangeSalvador(node.left, min, max)

        if max>node.elem:
            self._sumInsideRangeSalvador(node.right,min, max)
    ...
    def sumInsideRangeDum(self, min: int, max: int) -> int:
        return self._sumInsideRangeDum(self._root, min, max)
    ...
    def _sumInsideRangeDum(self, node: BinaryNode, min: int, max: int) -> int:
        if node is None:
            return 0
    ...
    # Sum value of all nodes (NOT LEAF) having value inside the given range
    def _sumInsideRangeOrder(self, node: BinaryNode, min: int, max: int) -> int:
        # Base Case
        if node is None:
            # node is empty
            return 0
            ...
            if (node.left is None) and (node.right is None):
                return 0
            if (node.elem > max) and node.left:
                self._sumInsideRangeOrder(node.left, min, max)

            if (min <= node.elem <= max) and (node.left or node.right):
                return node.elem + self._sumInsideRangeOrder(node.left, min, max) + self._sumInsideRangeOrder(node.right, min, max)

            if (node.elem < min) and node.right:
                self._sumInsideRangeOrder(node.right, min, max)
