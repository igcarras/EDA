...
class BinaryTree:
    ...
    """
    a recursive method that sums all the elements in the tree and returns this result.
    """
    def sumEtos(self):

        return self._sumEtos(self._root)
        
    def _sumEtos(self,node):
        if node is None:
            return 0
        
        return node.elem + self._sumEtos(node.left)	+ self._sumEtos(node.right)


    def smallest(self, temp):  
        ...
        if(self._root == None):  
            ...
            minimum = temp.elem;  
            if(temp.left != None):  
                minimum = min(minimum, self.smallest(temp.left));  
            if(temp.right != None):  
                minimum = min(minimum, self.smallest(temp.right));
    ...
    def identical(self, node):  
        ...
        if(self._root == None and node == None):  
            return True;  

        if(self._root == None or node == None):  
            return False;  

        if(self._root != None and node != None):
            
            return ((self._root.elem == node.elem) and 
                    (self.identical(node.left)) and 
                    (self.identical(node.right)));



    # Given two trees, return true if they are identical
    def identicalTrees(self, tree):
        
        return self._identicalTrees(self._root, tree._root)

    
    def _identicalTrees(self, node1, node2):
        ...
        if node1 is not None and node2 is not None:
    	    return ((node1.elem == node2.elem) and
    				self._identicalTrees(node1.left, node2.left) and
    				self._identicalTrees(node1.right, node2.right))
...
if __name__ == '__main__':
    ...
    print ('sum all the elements =', tree.sumEtos())
    print ('smallest =', tree.smallest(tree._root))
    print ('identical =', tree.identical(tree1._root))
    
    tree.draw()
    tree1.draw()
    print(tree.identicalTrees(tree1))
